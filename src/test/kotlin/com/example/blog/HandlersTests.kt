...
class HandlersTests(@Autowired val mockMvc: MockMvc) {
    @SpykBean
    ...
    fun `List articles`() {
        ...
        mockMvc.perform(get("/api/v2/article/").accept(MediaType.APPLICATION_JSON))
    ...
    fun `List users`() {
        ...
        mockMvc.perform(get("/api/v2/user/").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk)
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
